...
/* Idempotent, but not thread-safe: call it once from app_main before the tasks
 * that use a decoder are created (afsk_decoder_init calls it too, which covers
 * the single-decoder case and the host tests). */
