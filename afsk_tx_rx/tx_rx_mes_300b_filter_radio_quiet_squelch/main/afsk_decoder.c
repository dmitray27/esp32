...
/* Automatic squelch calibration. Radio audio levels differ from set to set,
 * and a receiver with its volume up delivers hiss that a fixed threshold may
 * not clear, so the idle line is measured and the squelch is parked
 * AUTO_SQUELCH_MARGIN above it. Only windows that contain nothing but idle
 * are used, so a transmission never raises the threshold, and the result is
 * never allowed below MIN_TONE_AMPLITUDE: calibration can only make the
 * receiver more selective than the hard floor, never less.
 * The switch itself lives in afsk_common.h. */
#define AUTO_SQUELCH_MARGIN 4.0f                /* +12 dB over idle */
#define AUTO_SQUELCH_MIN    MIN_TONE_AMPLITUDE
#define AUTO_SQUELCH_MAX    0.1f                /* -20 dBFS */
#define AUTO_SQUELCH_MS     3000u

/* Input overload: the ADC has no gain control, so clipping can only be fixed
 * by the operator turning the radio down. */
#define CLIP_LEVEL          0.98f
#define CLIP_REPORT_MS      30000u
...
void afsk_decoder_init(afsk_decoder_t *dec, int sample_rate) {
    ...
    dec->squelch_amp = MIN_TONE_AMPLITUDE;
    dec->squelch_power = MIN_TONE_POWER;
    dec->cal_time = dec->last_signal_time;
    dec->clip_report_time = dec->last_signal_time;
...
bool afsk_decoder_process_sample(afsk_decoder_t *dec,
    ...
    if (mag >= CLIP_LEVEL) dec->clip_count++;
    if (dec->clip_count && current_time - dec->clip_report_time >= CLIP_REPORT_MS) {
        printf("[LEVEL] input clipping (%" PRIu32 " samples at full scale) - turn the volume down\n",
               dec->clip_count);
        dec->clip_count = 0;
        dec->clip_report_time = current_time;
    }
...
#if AFSK_AUTO_SQUELCH
    /* Idle means: not decoding a frame and not counting anything that could
     * be a real preamble. Noise makes the odd mark bit appear, so a few ones
     * are tolerated; anything longer marks the window as contaminated. */
    if (dec->state != WAITING_FOR_PREAMBLE || dec->ones_count > PREAMBLE_DETECT / 4) {
        dec->cal_dirty = true;
    } else if (max_power > dec->cal_peak) {
        dec->cal_peak = max_power;
    }
    if (current_time - dec->cal_time >= AUTO_SQUELCH_MS) {
        if (!dec->cal_dirty) {
            float amp = 2.0f * sqrtf(dec->cal_peak) * AUTO_SQUELCH_MARGIN;
            if (amp < AUTO_SQUELCH_MIN) amp = AUTO_SQUELCH_MIN;
            if (amp > AUTO_SQUELCH_MAX) amp = AUTO_SQUELCH_MAX;
#ifdef LEVEL_REPORT_MS
            if (amp > dec->squelch_amp * 1.12f || amp < dec->squelch_amp * 0.89f) {
                printf("[SQUELCH] %.1f dBFS (idle %.1f dBFS)\n",
                       level_dbfs(amp), level_dbfs(2.0f * sqrtf(dec->cal_peak)));
            }
#endif
            dec->squelch_amp = amp;
            dec->squelch_power = amp * amp / 4.0f;
        }
        dec->cal_time = current_time;
        dec->cal_peak = 0.0f;
        dec->cal_dirty = false;
    }
#endif
    ...
    if (dec->signal_threshold < dec->squelch_power) {
        dec->signal_threshold = dec->squelch_power;
    }
