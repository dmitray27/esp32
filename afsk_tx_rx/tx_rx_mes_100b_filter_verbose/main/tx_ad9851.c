...
static volatile bool mutex_held = false;
...
bool tx_ad9851_send_block(const uint8_t *data, size_t len) {
    ...
    tx_bit_r_idx = 0;
    tx_bit_w_idx = 0;
    memset((void*)tx_bit_buffer, 0, TX_BIT_BUFFER_SIZE);

    ESP_LOGI(TAG, "Sending block: %zu bytes", len);

    afsk_serialize_block(data, len, tx_bit_buffer, &tx_bit_w_idx, TX_BIT_BUFFER_SIZE);
...
void tx_ad9851_wait_idle(void) {
    if (tx_is_active && tx_idle_sem) {
        xSemaphoreTake(tx_idle_sem, portMAX_DELAY);
    }
